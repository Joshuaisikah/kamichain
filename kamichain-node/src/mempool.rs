...
impl Mempool {
    ...
    pub fn add(&mut self, tx: Transaction, sender_balance: u64) -> Result<(), KamiError> {
        ...
        let pub_key_hex = tx.pub_key.as_ref().ok_or_else(|| {
            KamiError::InvalidTransaction("transaction is not signed".into())
        })?;
        Wallet::verify_transaction(&tx, pub_key_hex)
            .map_err(|e| KamiError::InvalidTransaction(e.to_string()))?;

        let total = tx.amount.checked_add(tx.fee).ok_or_else(|| {
            KamiError::InvalidTransaction("amount + fee overflow".into())
        })?;
        ...
        if self.pending.contains_key(&tx.id) {
            return Err(KamiError::InvalidTransaction("duplicate transaction".into()));
        }
