...
fn single_transaction_merkle_root_is_its_own_hash() {
    ...
    let tree   = MarkleTree::new(hashes);
...
fn two_identical_transactions_produce_consistent_root() {
    ...
    let tree   = MarkleTree::new(hashes.clone());
    let tree2  = MarkleTree::new(hashes);
    assert_eq!(tree.root(), tree2.root());
}

#[test]
fn different_transactions_produce_different_roots() {
    let tree_a = MarkleTree::new(vec!["tx1".to_string(), "tx2".to_string()]);
    let tree_b = MarkleTree::new(vec!["tx1".to_string(), "tx3".to_string()]);
...
fn order_of_transactions_matters() {
    let tree_a = MarkleTree::new(vec!["tx1".to_string(), "tx2".to_string()]);
    let tree_b = MarkleTree::new(vec!["tx2".to_string(), "tx1".to_string()]);
    assert_ne!(tree_a.root(), tree_b.root());
}

#[test]
fn root_is_64_hex_chars() {
    let tree = MarkleTree::new(vec!["a".to_string(), "b".to_string(), "c".to_string()]);
    let root = tree.root();
