...
```
tx_id = SHA-256( sender_bytes ∥ recipient_bytes ∥ amount_string_bytes ∥ nonce_string_bytes )
```

The four fields are fed into the hasher one after another — `sha2` accumulates them before finalising, so the result is identical to hashing a single concatenated byte string.
...
recipient = "bob"
amount    = 100
nonce     = 11742813690569277291   (random u64)

tx_id = SHA-256("alice" ++ "bob" ++ "100" ++ "11742813690569277291")
      → some 64-char hex string
```

`nonce` is a random `u64` generated by `rand::random()` inside `Transaction::new`. Because the nonce space is 2^64, two transactions with identical sender/recipient/amount will almost certainly produce different IDs.

The method on the struct delegates to the free function using the stored nonce:
...
|-------------|---------|--------|
| Transaction ID | `SHA-256(sender ∥ recipient ∥ amount_str ∥ nonce_str)` | `transaction.rs::compute_id` |
