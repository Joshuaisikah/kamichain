...
impl Wallet {
   ...
   pub  fn address(&self) -> String {
    ...
    pub fn verify_transaction( tx: &Transaction,pub_key_hex:&str) -> Result<bool, WalletError> {
        ...
        let pub_key_array: [u8;32]=pub_key_bytes.try_into().map_err(|_| WalletError::InvalidPublicKey("Key must be 32 bytes".into()))?;
        let mut hasher = Sha256::new();
        ...
        let verifying_key = VerifyingKey::from_bytes(&pub_key_array).map_err(|_| WalletError::InvalidPublicKey("Invalid public key bytes".into()))?;
        let message = format!("{}{}{}{}", tx.sender, tx.recipient, tx.amount, tx.id);
        let sig_bytes =hex::decode(sig_hex)?;
        let sig_array: [u8;64]=sig_bytes.try_into().map_err(|_| WalletError::VerificationFailed)?;
        let signature =Signature::from_bytes(&sig_array);
        verifying_key.verify(&message.as_bytes(), &signature).map_err(|_| WalletError::VerificationFailed)?;
        Ok(true)
        }
    ...
    pub fn load_from_file(path: impl AsRef<Path>) -> Result<Wallet, WalletError> {
        let hex = std::fs::read_to_string(path).map_err(|e| WalletError::InvalidPublicKey(e.to_string()))?;
        let bytes = hex::decode(hex.trim())?;
        let array: [u8; 32] = bytes.try_into().map_err(|_| WalletError::InvalidPublicKey("Key must be 32 bytes".into()))?;
